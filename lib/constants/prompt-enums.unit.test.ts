...
import { QuestionDifficulty, QuestionField, QuestionType } from './prompt-enums';
...
describe('Enum Sync Test', () => {
  ...
  it('enums in Prisma schema and TypeScript code are in sync', () => {
    ...
    const questionDifficultyMatch = schema.match(/enum QuestionDifficulty \{([^}]+)\}/);

    expect(questionTypeMatch).toBeTruthy();
    expect(questionFieldMatch).toBeTruthy();
    expect(questionDifficultyMatch).toBeTruthy();

    const prismaQuestionType = questionTypeMatch ? questionTypeMatch[1].trim().split(/\s+/) : [];
    const prismaQuestionField = questionFieldMatch ? questionFieldMatch[1].trim().split(/\s+/) : [];
    const prismaQuestionDifficulty = questionDifficultyMatch ? questionDifficultyMatch[1].trim().split(/\s+/) : [];
    ...
    expect(prismaQuestionDifficulty).toEqual(Object.values(QuestionDifficulty));
  });
  ...
  it('Question model has correct fields with correct types', () => {
    ...
    const questionModel = questionModelMatch ? questionModelMatch[1] : '';
